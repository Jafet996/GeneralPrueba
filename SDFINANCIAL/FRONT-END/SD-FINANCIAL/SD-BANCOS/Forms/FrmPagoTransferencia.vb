Imports BUSINESS
Public Class FrmPagoTransferencia
#Region "Variables"
    Private Numerico() As TNumericFormat
    Private _Titulo As String = "Transferencia"
...
#Region "Formateo de Campos"
    Private Sub FormateaCamposNumericos()
        Try
            ...
            Numerico(0) = New TNumericFormat(TxtTipoCambio, 5, 2, False, "0", "#,##0.00")
...
#End Region
    ...
    Public Sub Execute()
        Try
            CargaComboBancoEmpresa()
            CargaComboBancoProveedor()
            CargaDatos()

            Me.GbEmpresa.Text = EmpresaInfo.Nombre
            Me.Text = _Titulo
            Me.ShowDialog()
        Catch ex As Exception
            MensajeError(ex.Message)
    ...
    Private Sub CargaComboBancoEmpresa()
    ...
    Private Sub CargaComboBancoProveedor()
    ...
    Private Sub CargaComboCuentaProveedor()
        Dim ProveedorCuenta As New TProveedorCuenta
    ...
    Private Sub AsignaMonedaCuenta()
        ...
        Try
            ...
            If CmbMoneda.SelectedIndex <> CmbMonedaBeneficiario.SelectedIndex Then
                TxtTipoCambio.Text = TipoCambioBancos()
    ...
    Private Sub ComprobanteTransferencia(pBacoPago_Id As Long)
        Dim Pago As New TBcoPago
        Dim Reporte As New RptComprobanteTransferencia
        ...
        Try
            ...
            End With
            VerificaMensaje(Pago.RptComprobanteTransferencia)
    ...
    Private Function ValidaTodo() As Boolean
        ...
        Try
            ...
            If CmbBancoProveedor.SelectedIndex = -1 Then
                VerificaMensaje("Debe de seleccionar el banco del beneficiario")
            End If

            If CmbCuentaBeneficiario.SelectedIndex = -1 Then
                VerificaMensaje("Debe de seleccionar la cuenta a la cual realizó la transferencia")
            End If

            If TxtConcepto.Text.Trim = "" Then
                TxtConcepto.Focus()
                VerificaMensaje("Debe de ingresar un concepto a la transferencia")
            End If

            If TxtTransferencia.Text.Trim = "" Then
    ...
    Private Sub Aplicar()
        Dim BcoPago As New TBcoPago

        Try
            If MsgBox("¿Desea aplicar la transferencia?", MsgBoxStyle.Question + MsgBoxStyle.OkCancel, Me.Text) <> MsgBoxResult.Ok Then
                Exit Sub
            End If

            With BcoPago.Transferencia
                .Banco_Id = CInt(CmbBancoProveedor.SelectedValue)
                .Cuenta = CmbCuentaBeneficiario.Text.Trim
                .Moneda = IIf(CmbMonedaBeneficiario.SelectedIndex = 0, "C", "D")
                .Fecha = DateValue(DtpFechaTransferencia.Value)
                .TipoCambio = If(.Moneda = coMonedaDolares, TxtTipoCambio.Text.Trim, 1)
                .Numero = TxtTransferencia.Text.Trim
            End With

            With BcoPago
                .Emp_Id = EmpresaInfo.Emp_Id
                .TipoPago_Id = Enum_BcoTipoPago.Transferencia
                .Prov_Id = _Prov_Id
            ...
            _Acepto = True
            ComprobanteTransferencia(BcoPago.BcoPago_Id)
            Mensaje("La transferencia se aplicó de manera exitosa")
            Me.Close()
        Catch ex As Exception
            MensajeError(ex.Message)
        Finally
    ...
    Private Sub CmbBancoBeneficiario_SelectedValueChanged(sender As Object, e As EventArgs) Handles CmbBancoProveedor.SelectedValueChanged
        CargaComboCuentaProveedor()
    End Sub
