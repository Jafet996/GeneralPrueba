Imports Business
Public Class FrmBusquedaFacturasDev
    Private _Respuesta As TFacturaLlave
    Private _TipoDoc_Id As Enum_TipoDocumento
    ...
    Public Sub Execute(pTipoDoc_Id As Enum_TipoDocumento)
        Try

            _TipoDoc_Id = pTipoDoc_Id
            _Respuesta = Nothing
            ConfiguraDetalle()
            RdbFecha.Checked = True

            DtpFechaHasta.Value = DateValue(EmpresaInfo.Getdate())
            DtpFechaDesde.Value = DateAdd(DateInterval.Day, -7, DateValue(DtpFechaHasta.Value))
    ...
    Private Sub HabilitaControles()
        ...
        DtpFechaDesde.Enabled = RdbFecha.Checked
    ...
    Private Sub Buscar()
        ...
        Try
            ...
            VerificaMensaje(FacturaEncabezado.BusquedaFacturasDev(Tipo, TipoDoc_Id, DateValue(DtpFechaDesde.Value), DateValue(DtpFechaHasta.Value)))
    ...
    Private Function ValidaTodo() As Boolean
        Try
            ...
            If RdbFecha.Checked Then
                If DateValue(DtpFechaDesde.Value) > DateValue(DtpFechaHasta.Value) Then
                    VerificaMensaje("La fecha inicial no puede ser mayor a la fecha final")
                End If
            End If
